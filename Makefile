...
BIN_NAME=cidr-test-service

VERSION := $(shell grep "const Version " version.go | sed -E 's/.*"(.+)"$$/\1/')
GIT_COMMIT=$(shell git rev-parse HEAD)
GIT_DIRTY=$(shell test -n "`git status --porcelain`" && echo "+CHANGES" || true)
IMAGE_NAME := "divergentdave/cidr-test-service"

default: test

help:
	@echo 'Management commands for cidr-test-service:'
	@echo
	@echo 'Usage:'
...
build-docker:
	@echo "building ${BIN_NAME} ${VERSION}"
	docker build -t cidr-test-service:build -f Dockerfile.build .
	docker run --name=cidr-test-service -v $(GOPATH):/gopath/  cidr-test-service:build
	docker rm -f cidr-test-service
